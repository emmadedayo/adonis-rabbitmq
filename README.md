...
`@emmadedayo/adonis-rabbitmq` is a RabbitMQ provider for [Adonis](https://github.com/adonisjs/core).

## Getting Started

Instal `@emmadedayo/adonis-rabbitmq`:

```
yarn add @emmadedayo/adonis-rabbitmq
```

Then:

```
node ace invoke @emmadedayo/adonis-rabbitmq
```
...
Make sure to set the correct values to the enviroment variables so `@emmadedayo/adonis-rabbitmq` can connect.
...
Route.get('/', async () => {
  ...
  await Rabbit.sendToQueue('my_queue', 'This message was sent by @emmadedayo/adonis-rabbitmq')
})
```
...
When consuming messages through [`consumeFrom`](https://github.com/jotaajunior/@emmadedayo/adonis-rabbitmq#consumefrom), you'll receive in the callback a Message instance.
